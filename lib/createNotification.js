...
function buildCopy(type, metadata) {
  ...
  switch (type) {
    ...
    case 'NEW_CONTENT_FROM_CREATOR': {
      const name = m.creatorName ? String(m.creatorName) : 'Un creador';
      const preview = m.previewText ? String(m.previewText) : 'Nuevo contenido';
      return {
        title: `${name} publicó contenido nuevo`,
        message: preview
      };
    }
    case 'SYSTEM':
...
async function createNotification(data) {
  try {
    ...
    if (dedupeKey) {
      const row = await db.prepare('SELECT id FROM notifications WHERE dedupe_key = ?').get(dedupeKey);
      if (row) {
        return null;
      }
    }
