...
async function handleAvatarUpload(req, res) {
  ...
  db.prepare(`UPDATE users SET avatar_url = ?, updated_at = datetime('now') WHERE id = ?`).run(relativeUrl, req.user.id);

  if (previous && previous !== relativeUrl) {
    unlinkOldAvatar(previous);
  }

  const u = db.prepare('SELECT avatar_url, updated_at FROM users WHERE id = ?').get(req.user.id);
