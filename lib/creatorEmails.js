...
async function sendMissedRevenueAlertEmail(db, creator, fanName) {
  ...
  const row = await db
    .prepare(
      `SELECT MAX(sent_at) AS last_sent FROM email_logs WHERE user_id = ?::uuid AND type = ?`
    )
...
async function sendNewSubscriberEmail(db, creator, fan, plan, subscriptionId) {
  ...
  const dup = await db
    .prepare('SELECT id FROM email_logs WHERE user_id = ?::uuid AND type = ?')
    .get(creator.id, logType);
  if (dup) return;
